...
import { copyFile, lstat, mkdir, readlink, realpath, stat as followStat, symlink } from "node:fs/promises";
...
export async function copyIgnored(sourcePath, targetPath) {
  ...
  for (const name of selected) {
    ...
    if (sourceStat?.isSymbolicLink()) {
      const linkedStat = await followStat(from).catch(error => {
        if (error.code === "ENOENT") throw new Error(`Dangling source symlink: ${name}`);
        throw error;
      });
      if (!linkedStat.isFile()) continue;
      const original = await readlink(from);
      linkTarget = isAbsolute(original) ? original : `${relative(dirname(to), dirname(from))}/${original}`;
    } else {
    ...
    planned.push({ name, from, to, linkTarget });
  }

  for (const { from, to, linkTarget } of planned) {
    await mkdir(dirname(to), { recursive: true });
    if (linkTarget === undefined) await copyFile(from, to, constants.COPYFILE_EXCL);
    else await symlink(linkTarget, to);
  }
